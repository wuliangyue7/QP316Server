...
protected:
	...
	int								m_iRoomCardFee;						//房卡费用
	bool							m_bPerformRoomTaskCard;				//是否已推进房卡消耗任务
...
public:
	...
	TCHAR							m_szRoomGUID[PERSONAL_ROOM_GUID];	// 约战房间唯一标识
...
public:
	...
	//私人编号
	virtual void SetPersonalTableID(LPCTSTR szTableID);
	//房主强制解散房间
	virtual void HostDissumeGame(BOOL bIsEnforce = false);

	//获取约战房间
	virtual TCHAR * GetPersonalTableID();
...
public:
	...
	//设置房卡
	void SetRoomCardFee(int iRoomCard){m_iRoomCardFee=iRoomCard;}
};
