...
bool CDataBaseEngineSink::OnRequestRoomTaskProgress(DWORD dwContextID, VOID * pData, WORD wDataSize, DWORD &dwUserID)
{
	try
	{
		...
		m_PlatformDBAide.AddParameter(TEXT("@wKindID"), pRoomTask->wKindID);
		m_PlatformDBAide.AddParameter(TEXT("@lCreateRoomCount"), pRoomTask->nCreateRoomCount);
		m_PlatformDBAide.AddParameter(TEXT("@lUseCardCount"), pRoomTask->nUseCardCount);
		m_PlatformDBAide.AddParameter(TEXT("@lDrawCount"), pRoomTask->nDrawCount);
...
VOID CDataBaseEngineSink::OnLogonDisposeResult(DWORD dwContextID, DWORD dwErrorCode, LPCTSTR pszErrorString, bool bMobileClient,BYTE cbDeviceType,WORD wBehaviorFlags,WORD wPageTableCount)
{
	if (dwErrorCode==DB_SUCCESS)
	{
		...
		{
			TCHAR szInfo[260] = {0};
			wsprintf(szInfo, TEXT("%s   %s 出错"),   AnsiToUnicode(__FUNCTION__),  TEXT("GSP_GR_QueryTaskInfo"));
			CTraceService::TraceString(szInfo, TraceLevel_Exception);
		}
