...
bool CTableFrame::StartGame()
{
	...
	if (bGameStarted==false)
	{
		...
		for (WORD i=0;i<m_wChairCount;i++)
		{
			...
			if (pIServerUserItem!=NULL)
			{
				...
				if(m_pGameServiceOption->wServerType == GAME_GENRE_PERSONAL &&  m_szPersonalTableID != NULL )		
				{
					...
					{
						m_pIPersonaTableFrameHook->PersonalRoomWriteJoinInfo(pIServerUserItem->GetUserID(),  m_wTableID,  i,  m_pGameServiceOption->wKindID, m_szPersonalTableID, m_szRoomGUID);
					}

					//任务推进
					if(pIServerUserItem->GetUserID()==m_dwTableOwnerID && m_bPerformRoomTaskCard==false)
					{
						//定义变量
						CAttemperEngineSink * pAttemperEngineSink=(CAttemperEngineSink *)m_pIMainServiceFrame;
						if (pAttemperEngineSink!=NULL)
						{
							m_bPerformRoomTaskCard=true;
							pAttemperEngineSink->PerformRoomTaskProgress(pIServerUserItem,0,m_iRoomCardFee,0);
						}
		...
		if(m_dwPersonalPlayCount == 1)
		{
			...
			//m_wChairCount = nPlayerCount;
			//m_pGameServiceAttrib->wChairCount = nPlayerCount;
			//m_PersonalTableParameter.wJoinGamePeopleCount = nPlayerCount;
		}
		//游戏开始非逃跑状态
		memset(m_bEscapeStatus, 0, sizeof(m_bEscapeStatus));
...
bool CTableFrame::ConcludeGame(BYTE cbGameStatus)
{
	...
	if ((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0)
	{
		...
		//if(m_pIPersonaTableFrameHook!=NULL && m_pIPersonaTableFrameHook->OnEventGameEnd(m_wTableID,  m_wChairCount, m_dwDrawCountLimit, m_dwPersonalPlayCount, m_nSpecialInfoLen, m_cbSpecialInfo, m_PersonalUserScoreInfo))
		if(m_pIPersonaTableFrameHook!=NULL && m_pIPersonaTableFrameHook->OnEventGameEnd(m_wTableID,  m_wChairCount, m_dwDrawCountLimit, m_dwPersonalPlayCount, m_nSpecialInfoLen, m_cbSpecialInfo, m_sysStartTime, m_PersonalUserScoreInfo))
		{
			//桌子设置
			m_dwPersonalPlayCount = 0;
			m_dwTableOwnerID = 0;
			...
			for (WORD i=0;i<m_wChairCount;i++)
			{
				//获取用户
				IServerUserItem * pIServerUserItem=GetTableUserItem(i);			
				if (pIServerUserItem!=NULL&&pAttemperEngineSink!=NULL&&pIServerUserItem->GetUserID()==m_dwTableOwnerID)
				{
					//任务推进
					pAttemperEngineSink->PerformRoomTaskProgress(pIServerUserItem,1,0,0);
					break;
				}
			}
		}
...
SCORE CTableFrame::CalculateRevenue(WORD wChairID, SCORE lScore)
{
	...
	if ((m_pGameServiceOption->wRevenueRatio>0 || m_PersonalRoomOption.lPersonalRoomTax > 0)&&(lScore>=REVENUE_BENCHMARK))
	{
		...
		if(lstrcmp(m_pGameServiceOption->szDataBaseName,  TEXT("RYGameScoreDB")) == 0)
		{
				m_pGameServiceOption->wRevenueRatio = 0;
				m_PersonalRoomOption.lPersonalRoomTax = 0;
		}
...
bool CTableFrame::OnEventUserOffLine(IServerUserItem * pIServerUserItem)
{
	...
	if (cbUserStatus!=US_LOOKON)
	{
		...
		if ((cbUserStatus==US_PLAYING)&&(m_wOffLineCount[wChairID]<MAX_OFF_LINE) || 
			...
			(m_PersonalTableParameter.cbIsJoinGame && m_dwTableOwnerID == pUserInfo->dwUserID ||  //必须参与模式下房主不能退出游戏
			(m_dwPersonalPlayCount != 0 && m_dwDrawCountLimit > m_dwPersonalPlayCount) ||				//游戏已经开始不能退出游戏
			(cbUserStatus >= US_SIT))																												//玩家只要坐下就不能退出游戏
			)
		{
			//用户设置
...
{
	return m_dwPersonalPlayCount;//m_wDrawCount;
}
...
bool CTableFrame::PerformStandUpAction(IServerUserItem * pIServerUserItem, bool bInitiative/*=false*/)
{
	...
	if (
		...
		(
			...
			(m_PersonalTableParameter.cbIsJoinGame || m_dwPersonalPlayCount != 0 || cbUserStatus >= US_SIT) && !m_bIsDissumGame &&//如果房主参与游戏,且不是强制解散游戏
			(m_pGameServiceOption->wServerType==GAME_GENRE_PERSONAL && (m_dwPersonalPlayCount != 0  || (m_dwTableOwnerID == pIServerUserItem->GetUserID() && 
			((m_dwDrawCountLimit!=0 && m_dwDrawCountLimit <= m_dwPersonalPlayCount)	) )))	
		)
	   )
	{	
		//比赛类型，约战房间类型
		if((m_pGameServiceOption->wServerType==GAME_GENRE_MATCH && m_pGameMatchOption->cbMatchType==MATCH_TYPE_IMMEDIATE)||	m_pGameServiceOption->wServerType==GAME_GENRE_PERSONAL && 
			(m_PersonalTableParameter.cbIsJoinGame || cbUserStatus >= US_SIT ||(m_dwPersonalPlayCount != 0  && (m_dwDrawCountLimit!=0 && m_dwDrawCountLimit  >= m_dwPersonalPlayCount)))
		  )
		{
...
bool CTableFrame::PerformSitDownAction(WORD wChairID, IServerUserItem * pIServerUserItem, LPCTSTR lpszPassword)
{
	...
	{
		//发送信息
		SendRequestFailure(pIServerUserItem,TEXT("约战房间已经解散！"),REQUEST_FAILURE_NORMAL);
		return false;
	}
	...
	//如果是私人房，且已经开始游戏，则不允许其他人入座
	if (((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0) && m_dwPersonalPlayCount != 0)
	{
		...
		if(!bUserExist)
		{
			...
			LogPrintf(("userid = %d  m_wChairCount = %d  GetSitUserCount() = %d  m_dwPersonalPlayCount = %d  m_wTableID = %d 约战已经开始"),  pIServerUserItem->GetUserID() , m_wChairCount, GetSitUserCount(), m_dwPersonalPlayCount, m_wTableID );
		}
	...
	if((m_pGameServiceOption->wServerType & GAME_GENRE_PERSONAL) != 0)
	{
		...
		if (!bTreasureDB  && !bIsExsit)
		{
			if (ZJH_KIND_ID== m_pGameServiceOption->wKindID || NN_KIND_ID == m_pGameServiceOption->wKindID || TBZ_KIND_ID  == m_pGameServiceOption->wKindID)
			{
				...
				pIServerUserItem->ModifyUserInsure(lScore, 0, 0);
				m_lIniScore = lMinScore;
			}
			else
			{
...
bool CTableFrame::EfficacyStartGame(WORD wReadyChairID)
{
	...
	case START_MODE_ALL_READY:			//所有准备
		...
		{
			if (wReadyUserCount == m_nPersonalRoomPlayerCount)
			{
				return true ;
			}
		}
...
bool CTableFrame::CancelTableRequestReply(DWORD dwUserID, BYTE cbAgree)
{
	...
	if(wAgreeCount >= wOnlineCount || wAgreeCount >= ((m_wChairCount + 1)/2 ) || wAgreeCount >= ((m_nPersonalRoomPlayerCount + 1)/2 ) ) bCancel = true;
	...
	if(bCancel == true)
	{
		...
		//推进任务 
		if(m_bPerformRoomTaskCard==true)
		{
			//定义变量
			CAttemperEngineSink * pAttemperEngineSink=(CAttemperEngineSink *)m_pIMainServiceFrame;
...
//游戏日志
void CTableFrame::WriteGameLog(LPCTSTR pszLogString)
{
	CTraceService::TraceString(pszLogString,TraceLevel_Exception);
}
//////////////////////////////////////////////////////////////////////////////////
