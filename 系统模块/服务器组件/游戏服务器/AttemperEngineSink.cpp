	...
	case CT_CONNECT_PERSONAL_ROOM_CORRESPOND:
		{
			...
			//构造提示
			TCHAR szString[512]=TEXT("");
			_sntprintf(szString,CountArray(szString),TEXT("正在连接约战服务器 [ %s:%d ]"),pPrsnlRmCorrespondAddress->szAddress,m_pInitParameter->m_wPrsnlRmCorrespondPort);
...
bool CAttemperEngineSink::OnEventDataBase(WORD wRequestID, DWORD dwContextID, VOID * pData, WORD wDataSize)
{
	...
	//约战（约战房）事件
	if(wRequestID>=DBO_GR_CREATE_SUCCESS && wRequestID<=DBO_GR_CURRENCE_ROOMCARD_AND_BEAN)
	{
		//参数效验
...
// 			//创建成功 房卡还没有消耗
// 			PerformRoomTaskProgress(pIServerUserItem,1,/*pCreateSuccess->iRoomCardFee*/0,0);
// 		}
...
bool CAttemperEngineSink::OnTCPNetworkSubQueryOption(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	if(lstrcmp(m_pGameServiceOption->szDataBaseName,  TEXT("RYGameScoreDB")) == 0)
	{
			OptionCurrent.ServerOptionInfo.wRevenueRatio = 0;
	}
...
bool CAttemperEngineSink::OnTCPNetworkSubOptionServer(VOID * pData, WORD wDataSize, DWORD dwSocketID)
{
	...
	if(lstrcmp(m_pGameServiceOption->szDataBaseName,  TEXT("RYGameScoreDB")) == 0)
	{
			...
			m_pGameServiceOption->lServiceScore=0;
	}
...
bool CAttemperEngineSink::PerformRoomTaskProgress(IServerUserItem * pIServerUserItem,tagUserTaskEntry * pUserTaskEntry,int iRoom,int iRoomNum,int iPlayCount)
{
	...
	PerformRoom.dwUserID = pIServerUserItem->GetUserID();
	PerformRoom.wKindID=m_pGameServiceOption->wKindID;
	PerformRoom.nCreateRoomCount=iRoom;
	PerformRoom.nUseCardCount=iRoomNum;
	PerformRoom.nDrawCount=iPlayCount;
	...
	//投递请求
	m_pIDBCorrespondManager->PostDataBaseRequest(pIServerUserItem->GetUserID(),DBR_GR_WRITE_ROOM_TASK_PROGRESS,dwSocketID,&PerformRoom,sizeof(PerformRoom));
...
//开始游戏写入参与信息
VOID CAttemperEngineSink::PersonalRoomWriteJoinInfo(DWORD dwUserID, WORD wTableID,  WORD wChairID,  DWORD dwKindID, TCHAR * szRoomID,  TCHAR * szPersonalRoomGUID)
{
	//退还费用
