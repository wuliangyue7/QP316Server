...
bool CGlobalInfoManager::AddServerTable(CString strServerID, tagPersonalTableInfo PersonalTable)
{
	...
	if (GetHostCreatePersonalRoomCount(PersonalTable.dwUserID,  PersonalTable.dwKindID) > MAX_CREATE_PERSONAL_ROOM)
	{
		return false;
	}
...
//获取房主创建的房间的数量
INT CGlobalInfoManager::GetHostCreatePersonalRoomCount(DWORD dwUserID,  DWORD dwKindID)
{
	tagPersonalTableInfo* pPersonalTable = NULL;
	POSITION Position = NULL;
	int iHostCreateRoomCount = 0;
	...
	{
		m_MapPersonalTableInfo.GetNextAssoc(Position,str,pPersonalTable);
		if(pPersonalTable != NULL && pPersonalTable->dwUserID == dwUserID &&  pPersonalTable->dwKindID == dwKindID)
		{
			iHostCreateRoomCount++;
		}
	}
...
//获取房间数目
DWORD CGlobalInfoManager::GetFreeServerCount(DWORD dwKindID)
{
	int iServerCount = 0;
	//查找房间
	INT_PTR nSize = m_ServerTableCountArray.GetCount();
	...
	{
		tagServerTableCount* pServerTableCount = m_ServerTableCountArray.GetAt(i);
		if(pServerTableCount->dwKindID == dwKindID)
		{
			iServerCount++;
		}
	}
...
bool CGlobalInfoManager::CanCreatePersonalRoom(DWORD dwKindID,   DWORD dwUserID)
{
	...
	//获得约战房间数目
	int iHaveCreate = GetHostCreatePersonalRoomCount(dwUserID, dwKindID);
