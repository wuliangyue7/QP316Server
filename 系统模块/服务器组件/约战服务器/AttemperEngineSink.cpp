	...
	case SUB_CS_C_QUERY_GAME_SERVER:		//查询可用房间
		{
			...
			if (m_GlobalInfoManager.GetFreeServerCount(pQueryGameServer->dwKindID) > 0)
			{
				...
				{
					lstrcpyn(QueryGameServerResult.szErrDescrybe, TEXT("已经达到创建约战房间的最大数，不能再创建房间！"),  sizeof(QueryGameServerResult.szErrDescrybe));
				}
			}
			else
			{
				lstrcpyn(QueryGameServerResult.szErrDescrybe, TEXT("当前游戏的约战房服务器不存在，请选择其他游戏！"),  sizeof(QueryGameServerResult.szErrDescrybe));
			}
