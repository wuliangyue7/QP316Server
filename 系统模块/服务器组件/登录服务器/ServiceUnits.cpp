...
bool CServiceUnits::ConcludeService()
{
	...
	if (m_PsrnlRmTCPSocketService.GetInterface()!=NULL) m_PsrnlRmTCPSocketService->ConcludeService();
...
bool CServiceUnits::InitializeService()
{
	...
	if ((m_PsrnlRmTCPSocketService.GetInterface()==NULL)&&(m_PsrnlRmTCPSocketService.CreateInstance()==false)) return false;
	...
	//约战服务
	if (m_PsrnlRmTCPSocketService->SetServiceID(NETWORK_PERSONAL_ROOM_CORRESPOND)==false) return false;
	if (m_PsrnlRmTCPSocketService->SetTCPSocketEvent(pIAttemperEngine)==false) return false;
	...
	m_AttemperEngineSink.m_pPrsnlRmITCPSocketService = m_PsrnlRmTCPSocketService.GetInterface();
...
bool CServiceUnits::StartKernelService()
{
	...
	//约战引擎
	if (m_PsrnlRmTCPSocketService->StartService()==false)
	{
		ASSERT(FALSE);
		return false;
	}
