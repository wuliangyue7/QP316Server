...
#define ITEM_SERVER_OPTION_PERONAL	5							//约战定制配置
...
BOOL CDlgServerOptionItem1::OnInitDialog()
{
	...
	if (m_pGameServiceOption->wServerType == GAME_GENRE_PERSONAL)
	{
		...
		else 	if ((lstrcmp(m_pGameServiceOption->szDataBaseName,  TEXT("RYGameScoreDB")) == 0))
		{
...
bool CDlgServerOptionItem1::SaveInputInfo()
{
	...
	{
		CServerRule::SetAllowDynamicJoin(m_pGameServiceOption->dwServerRule, true);
	}
...
bool CDlgServerOptionItem5::GetPersonalRoomInfo(DWORD dwKindID)
{
	...
	//获取配置参数
	m_pPersonalRoomOption->cbCardOrBean = IniData.ReadInt(TEXT("Option"),TEXT("CardOrBean"),0);
	m_pPersonalRoomOption->lFeeCardOrBeanCount = IniData.ReadInt(TEXT("Option"),TEXT("FeeCardOrBeanCount"),0);
	m_pPersonalRoomOption->cbIsJoinGame = IniData.ReadInt(TEXT("Option"),TEXT("IsJoinGame"),0);
	m_pPersonalRoomOption->cbMinPeople = IniData.ReadInt(TEXT("Option"),TEXT("MinPeople"),0);
	m_pPersonalRoomOption->cbMaxPeople = IniData.ReadInt(TEXT("Option"),TEXT("MaxPeople"),0);
	m_pPersonalRoomOption->lMaxCellScore = IniData.ReadInt(TEXT("Option"),TEXT("MaxCellScore"),0);
	m_pPersonalRoomOption->lPersonalRoomTax = IniData.ReadInt(TEXT("Option"),TEXT("PersonalRoomTax"),0);
	m_pPersonalRoomOption->wCanCreateCount = IniData.ReadInt(TEXT("Option"),TEXT("CanCreateCount"),0);
	m_pPersonalRoomOption->dwPlayTurnCount = IniData.ReadInt(TEXT("Option"),TEXT("PlayTurnCount"),0);
	m_pPersonalRoomOption->dwPlayTimeLimit = IniData.ReadInt(TEXT("Option"),TEXT("PlayTimeLimit"),0);
	m_pPersonalRoomOption->dwTimeAfterBeginCount = IniData.ReadInt(TEXT("Option"),TEXT("TimeAfterBeginCount"),0);
	m_pPersonalRoomOption->dwTimeOffLineCount = IniData.ReadInt(TEXT("Option"),TEXT("TimeOffLineCount"),0);
	m_pPersonalRoomOption->dwTimeNotBeginGame = IniData.ReadInt(TEXT("Option"),TEXT("TimeNotBeginGame"),0);
	m_pPersonalRoomOption->dwTimeAfterCreateRoom = IniData.ReadInt(TEXT("Option"),TEXT("TimeAfterCreateRoom"),0);


	//创建私人房间的权限
	TCHAR szPlayTurnLimit[50] = {0};
	TCHAR szPayFee[50] = {0};
	TCHAR szIniScore[50] = {0};


	m_pCreateRoomRightAndFee->cbCreateRight=IniData.ReadInt(TEXT("CreateRoomRight"), TEXT("CreateRight"), 0);

	for (int i = 0; i < FEE_OPTION_COUNT ; i++)
	{
		wsprintf(szPlayTurnLimit, TEXT("PlayTurnLimit_%d"),i);
		m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].dwPlayTurnLimit=IniData.ReadInt(TEXT("FeeParameter"),szPlayTurnLimit,0);

		wsprintf(szPayFee, TEXT("PayFee_%d"),i);
		m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].dwPayFee =IniData.ReadInt(TEXT("FeeParameter"),szPayFee,0);

		wsprintf(szIniScore, TEXT("IniScore_%d"),i);
		m_pCreateRoomRightAndFee->stCreateRoomPayFee[i].dwIniScore=IniData.ReadInt(TEXT("FeeParameter"),szIniScore,0);
...
BOOL CDlgServerOptionItemPersonal::OnInitDialog()
{
	...
	//创建窗口
	WORD wPersonalRoomRuleSize=sizeof(m_pGameServiceOption->cbPersonaRule);
	m_hPersonalRule=m_pIGameServicePersonalRule->CreatePersonalRule(this,rcClient,m_pGameServiceOption->cbPersonaRule, wPersonalRoomRuleSize);
...
bool CDlgServerOptionItemPersonal::SaveInputInfo()
{
	...
	{
		//保存设置
		WORD wPersonalSize=sizeof(m_pGameServiceOption->cbPersonaRule);
		bool bSuccess=m_pIGameServicePersonalRule->SavePersonalRule(m_pGameServiceOption->cbPersonaRule,wPersonalSize);
...
VOID CDlgServerWizard::SetWizardParameter(IGameServiceManager * pIGameServiceManager, tagGameServiceOption * pGameServiceOption)
{
	...
	if (pGameServiceOption==NULL)
	{
		...
		{
			WORD wPersonalRoomSize=sizeof(m_ModuleInitParameter.GameServiceOption.cbPersonaRule);
			m_pIGameServicePersonalRule->DefaultPersonalRule(m_ModuleInitParameter.GameServiceOption.cbPersonaRule,wPersonalRoomSize);
		}
...
VOID CDlgServerWizard::OnBnClickedFinish()
{
	...
	//税收配置
	if(lstrcmp(m_ModuleInitParameter.GameServiceOption.szDataBaseName,  TEXT("RYGameScoreDB")) == 0)
	{
		m_ModuleInitParameter.GameServiceOption.wRevenueRatio = 0;
		m_ModuleInitParameter.GameServiceOption.lServiceScore = 0;
	}
	...
	CopyMemory(GameServerCreate.cbPersonalRule,m_ModuleInitParameter.GameServiceOption.cbPersonaRule,sizeof(GameServerCreate.cbPersonalRule));
	...
	CopyMemory(&PersonalRule, m_ModuleInitParameter.GameServiceOption.cbPersonaRule, sizeof(PersonalRule));

	//配置特殊规则
	if(PersonalRule.cbSpecialRule == 1)
	{
		//房间费用配置
		for(int i = 0; i < PERSONAL_OPTION_COUNT; i++)
		{
			m_ModuleInitParameter.m_pCreateRoomRightAndFee.stCreateRoomPayFee[i].dwIniScore = PersonalRule.personalRule[i].nIniScore;
			if(m_ModuleInitParameter.m_pCreateRoomRightAndFee.stCreateRoomPayFee[i].dwIniScore <= 0)
			{
				m_ModuleInitParameter.m_pCreateRoomRightAndFee.stCreateRoomPayFee[i].dwIniScore = 1000;
			}
			m_ModuleInitParameter.m_pCreateRoomRightAndFee.stCreateRoomPayFee[i].dwPayFee = PersonalRule.personalRule[i].nFee;
			m_ModuleInitParameter.m_pCreateRoomRightAndFee.stCreateRoomPayFee[i].dwPlayTimeLimit = PersonalRule.personalRule[i].nPlayerCount;
			m_ModuleInitParameter.m_pCreateRoomRightAndFee.stCreateRoomPayFee[i].dwPlayTurnLimit = PersonalRule.personalRule[i].nTurnCount;
			m_ModuleInitParameter.m_pCreateRoomRightAndFee.stCreateRoomPayFee[i].nExOne = PersonalRule.personalRule[i].nExOne;
			m_ModuleInitParameter.m_pCreateRoomRightAndFee.stCreateRoomPayFee[i].nExTwo = PersonalRule.personalRule[i].nExTwo;
			m_ModuleInitParameter.m_pCreateRoomRightAndFee.stCreateRoomPayFee[i].nExThree = PersonalRule.personalRule[i].nExThree;
		}
