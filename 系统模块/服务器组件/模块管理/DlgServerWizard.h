...
public:
	bool GetPersonalRoomInfo(DWORD dwKindID);
};
...
//私人房间定制配置
class MODULE_MANAGER_CLASS CDlgServerOptionItemPersonal : public CDlgServerOptionItem
{
	//控件变量
protected:
	HWND											m_hPersonalRule;									//定制规则
	IGameServicePersonalRule *		m_pIGameServicePersonalRule;			//自定配置

	//函数定义
public:
	//构造函数
	CDlgServerOptionItemPersonal();
	//析构函数
	virtual ~CDlgServerOptionItemPersonal();
...
protected:
	...
	CDlgServerOptionItem *			m_pOptionItem[6];					//向导指针
...
protected:
	...
	CDlgServerOptionItemPersonal		m_ServerOptionItemPersonal;		//定制配置
	//函数定义
public:
	//构造函数
...
class MODULE_MANAGER_CLASS CDlgServerWizard : public CDialog
{
	...
	friend class CDlgServerOptionItemPersonal;
