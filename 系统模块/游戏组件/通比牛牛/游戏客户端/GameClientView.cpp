...
#define	IDC_ADMIN						108								//控制按钮
#define IDC_TRUSTEE					109								//托管按钮
//定时器标识
...
BEGIN_MESSAGE_MAP(CGameClientView, CGameFrameViewGDI)
	...
	ON_BN_CLICKED(IDC_TRUSTEE, OnBnClickedTrustee)
	ON_WM_LBUTTONDOWN()
	ON_WM_TIMER()
END_MESSAGE_MAP()
...
int CGameClientView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_btTrustee.Create(NULL,WS_CHILD,rcCreate,this,IDC_TRUSTEE);
	...
	m_btTrustee.SetButtonImage(IDB_BT_TRUSTEE,hInstance,false,false);
	m_btTrustee.ShowWindow(SW_SHOW);
	//创建控件
	for (WORD i=0;i<GAME_PLAYER;i++)
	{
...
VOID CGameClientView::RectifyControl(int nWidth, int nHeight)
{
	...
	DeferWindowPos(hDwp,m_btTrustee, NULL, nWidth/2+250, nHeight-55, 0, 0, uFlags);
	DeferWindowPos(hDwp,m_btHintOx, NULL, nWidth/2+250, nHeight-100, 0, 0, uFlags);
	DeferWindowPos(hDwp,m_btOpenCard, NULL,nWidth/2+250, nHeight-150, 0, 0, uFlags);
...
VOID CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	for (BYTE i = 0; i < GAME_PLAYER; i++)
	{
		...
		if (pIClientUserItem!=NULL)
		{
			if (m_cbOpenCardUser[i]==TRUE && i!=MY_VIEW_CHAIRID) m_PngOpenTag.DrawImage(pDC,m_ptOpenTag[i].x,m_ptOpenTag[i].y);
		}
	}
	...
	if (m_bShowScore==true)
	{
		...
		{			
			...
			//绘画信息
			if (i == MY_VIEW_CHAIRID) pDC->SetTextColor(RGB(255,255,0));
			...
			if (i == MY_VIEW_CHAIRID) pDC->SetTextColor(RGB(255,153,0));
...
{
	//发送引擎消息
	SendEngineMessage(IDM_ADMIN_WND, (WPARAM)m_nWidth, (LPARAM)m_nHeight);
}
	...
	case IDI_SEND_CARD:		//移动扑克
		{
			//设置索引
			m_wSendIndex++;
			m_wSendCount=3;//不管窗口大小，固定3步走完，避免最大化和普通窗口发牌速度不一致 lyy20160721
			//设置位置
			tagSendCardItem * pSendCardItem=&m_SendCardItemArray[0];
...
VOID CGameClientView::OnBnClickedTrustee()
{
	...
	else
	{
		...
		m_btTrustee.SetButtonImage(IDB_BT_UN_TRUSTEE,hInstance,false,false);
	}
}
