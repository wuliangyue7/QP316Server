...
#define IDI_TRUSTEE_OPEN			5									//托管延时开牌
#define IDI_TRUSTEE_START			6									//托管延时准备
//时间标识
	...
	case GS_TK_FREE:		//空闲状态
		{
			...
			if (IsLookonMode()==false)
			{
				if(GetMeUserItem()->GetUserStatus()!=US_READY)
				{
					...
					m_GameClientView.m_bTrustee=false;
					m_GameClientView.m_btTrustee.SetButtonImage(IDB_BT_TRUSTEE,AfxGetInstanceHandle(),false,false);
				}
...
bool CGameClientEngine::OnSubGameEnd(const void * pBuffer, WORD wDataSize)
{
	...
	for (WORD i = 0; i < GAME_PLAYER; i++)
	{
		if (i == GetMeChairID() && IsLookonMode() == false) continue;
		...
		if (m_cbHandCardData[i][0] > 0)
		{
			if (m_cbUserOxCard[i] == TRUE)
			{
				...
				//显示点数
				if (cbCardValue >= OX_VALUE_10) bOxSound = true;
...
m_pIStringMessage->InsertNormalString(TEXT("\n"));
	...
	if (IsLookonMode()==false )
	{
		...
		{
			if(m_GameClientView.m_bTrustee)//托管了
			{
				//OnStart(0,0);
				SetTimer(IDI_TRUSTEE_START,3000,NULL);
			}
			else
			{
				m_GameClientView.m_btStart.ShowWindow(SW_SHOW);
...
LRESULT CGameClientEngine::OnSendCardFinish(WPARAM wParam, LPARAM lParam)
{
	...
	if(!m_cbDynamicJoin)
	{
		...
		if(m_GameClientView.m_bTrustee)//托管时自动摊牌
		{
			//OnOpenCard(0,0);
			SetTimer(IDI_TRUSTEE_OPEN,1000,NULL);
		}
		else
		{
...
LRESULT	CGameClientEngine::OnOpenCard(WPARAM wParam, LPARAM lParam)
{
	...
	if (nCount == 0 && m_bTimeOut == false) OnHintOx(0, 0);
...
LRESULT CGameClientEngine::OnOxEnable(WPARAM wParam, LPARAM lParam)
{
	...
	if(bEnable==TRUE)
	{
			...
			cbCardData[1] != 0x4E && cbCardData[1] != 0x4F)
		{
			...
			switch (cbValue)
			{
			case 1: cbValue = OX_VALUE_BIG_1;break;
			case 2: cbValue = OX_VALUE_BIG_2;break;
			case 3: cbValue = OX_VALUE_BIG_3;break;
			case 4: cbValue = OX_VALUE_BIG_4;break;
			case 5: cbValue = OX_VALUE_BIG_5;break;
			case 6: cbValue = OX_VALUE_BIG_6;break;
			case 7: cbValue = OX_VALUE_BIG_7;break;
			case 8: cbValue = OX_VALUE_BIG_8;break;
			case 9: cbValue = OX_VALUE_BIG_9;break;
			case 0: cbValue = OX_VALUE_BIG_10;break;
			}
		}
		else
		{
...
void CGameClientEngine::OnTimer(UINT nIDEvent)
{
	...
	case IDI_TRUSTEE_START:
		{
			KillTimer(IDI_TRUSTEE_START);
			OnStart(0,0);
			return;
		}
	case IDI_TRUSTEE_OPEN:
		{
			KillTimer(IDI_TRUSTEE_OPEN);
