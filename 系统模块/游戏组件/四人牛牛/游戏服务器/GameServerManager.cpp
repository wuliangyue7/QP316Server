...
bool CGameServiceManager::RectifyParameter(tagGameServiceOption & GameServiceOption)
{
	...
	//判断是积分房卡
	if ((GameServiceOption.wServerType & GAME_GENRE_PERSONAL) && (lstrcmp(GameServiceOption.szDataBaseName,  TEXT("RYGameScoreDB")) == 0))
	{
		//设置文件名
		...
		//读取配置
		LONGLONG lRoomCardJetton = GetPrivateProfileInt(GameServiceOption.szServerName, TEXT("lJettonFour"), 80, szFileName);

		//积分房卡最低分配积分为  (人数-1) * 5（最大倍数） * 最大的可配置筹码
		GameServiceOption.lMinTableScore = (GAME_PLAYER - 1) * lRoomCardJetton * 5;
	}
