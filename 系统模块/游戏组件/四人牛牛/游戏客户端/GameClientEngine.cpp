...
BEGIN_MESSAGE_MAP(CGameClientEngine, CGameFrameEngine)
	...
	ON_MESSAGE(IDM_ADMIN_WND, OnAdminWnd)
	ON_MESSAGE(IDM_LBUTTONDOWN_VIEW, OnLButtonDownView)
...
CGameClientEngine::CGameClientEngine()
{
	...
	m_bRoomCardScore = false;
	ZeroMemory(m_lRoomCardJetton, sizeof(m_lRoomCardJetton));
	...
	case SUB_S_ADMIN_STORAGE_INFO:	//特殊客户端信息
		{
			...
			m_pClientControlDlg->SetStorageInfo(pStorage);
			return true;
		}
	case SUB_S_REQUEST_QUERY_RESULT:
		{
			...
			m_pClientControlDlg->RequestQueryResult(pQueryResult);
	...
	case SUB_S_USER_CONTROL:
		{
			...
			CMD_S_UserControl *pUserControl = (CMD_S_UserControl *)pBuffer;
			m_pClientControlDlg->RoomUserControlResult(pUserControl);
	...
	case SUB_S_USER_CONTROL_COMPLETE:
		{
			...
			CMD_S_UserControlComplete *pUserControlComplete = (CMD_S_UserControlComplete *)pBuffer;
			m_pClientControlDlg->UserControlComplete(pUserControlComplete);
	...
	case SUB_S_OPERATION_RECORD:
		{
			...
			m_pClientControlDlg->UpdateOperationRecord(pOperation_Record);
	...
	case SUB_S_REQUEST_UDPATE_ROOMINFO_RESULT:
		{
			...
			m_pClientControlDlg->UpdateRoomInfoResult(pRoomInfo_Result);
	...
	case GS_TK_FREE:		//空闲状态
		{
			...
			if (CUserRight::IsGameCheatUser(m_pIClientKernel->GetUserAttribute()->dwUserRight) && m_hInst)
			{
				m_GameClientView.m_btOpenAdmin.ShowWindow(SW_SHOW);
				
				//设置更新定时器
				m_pClientControlDlg->SetUpdateIDI();

				//设置房间库存
				m_pClientControlDlg->SetRoomStorage(pStatusFree->lRoomStorageStart, pStatusFree->lRoomStorageCurrent);
			}

			m_GameClientView.m_bIsAllowAvertCheat = pStatusFree->bIsAllowAvertCheat;
	...
	case GS_TK_CALL:	// 叫庄状态
		{
			...
			if (CUserRight::IsGameCheatUser(m_pIClientKernel->GetUserAttribute()->dwUserRight) && m_hInst)
			{
				...
				//设置房间库存
				m_pClientControlDlg->SetRoomStorage(pStatusCall->lRoomStorageStart, pStatusCall->lRoomStorageCurrent);
			}
			
			m_GameClientView.m_bIsAllowAvertCheat = pStatusCall->bIsAllowAvertCheat;
	...
	case GS_TK_SCORE:	//下注状态
		{
			...
			if (!IsLookonMode() && pStatusScore->lTurnMaxScore>0L && m_lTableScore[GetMeChairID()]==0L&&(!m_bDynamicJoin))
			{
				...
				//更新控件
				if (pStatusScore->bRoomCardScore == false)
				{
					UpdateScoreControl(lUserMaxScore,SW_SHOW);
				}
				else
				{
					UpdateScoreControl(pStatusScore->lRoomCardJetton,SW_SHOW);
				}

				m_bRoomCardScore = pStatusScore->bRoomCardScore;
				CopyMemory(m_lRoomCardJetton, pStatusScore->lRoomCardJetton, sizeof(m_lRoomCardJetton));
			...
			if (CUserRight::IsGameCheatUser(m_pIClientKernel->GetUserAttribute()->dwUserRight) && m_hInst)
			{
				...
				//设置房间库存
				m_pClientControlDlg->SetRoomStorage(pStatusScore->lRoomStorageStart, pStatusScore->lRoomStorageCurrent);
			}
			
			m_GameClientView.m_bIsAllowAvertCheat = pStatusScore->bIsAllowAvertCheat;
	...
	case GS_TK_PLAYING:	//游戏状态
		{
			...
			if (CUserRight::IsGameCheatUser(m_pIClientKernel->GetUserAttribute()->dwUserRight) && m_hInst)
			{
				...
				//设置房间库存
				m_pClientControlDlg->SetRoomStorage(pStatusPlay->lRoomStorageStart, pStatusPlay->lRoomStorageCurrent);
			}
			
			m_GameClientView.m_bIsAllowAvertCheat = pStatusPlay->bIsAllowAvertCheat;
...
bool CGameClientEngine::OnSubGameStart(const void * pBuffer, WORD wDataSize)
{
	...
	if (pGameStart->banker_config == DESPOT_BANKER || pGameStart->banker_config == RANDOM_BANKER)
	{
		//用户信息
		for (WORD i=0;i<GAME_PLAYER;i++)
		{
	...
	if (!IsLookonMode() && pGameStart->lTurnMaxScore>0)
	{
		...
		//更新控件
		if (pGameStart->bRoomCardScore == false)
		{
			UpdateScoreControl(lUserMaxScore,SW_SHOW);
		}
		else
		{
			UpdateScoreControl(pGameStart->lRoomCardJetton, SW_SHOW);
		}

		m_bRoomCardScore = pGameStart->bRoomCardScore;
		CopyMemory(m_lRoomCardJetton, pGameStart->lRoomCardJetton, sizeof(m_lRoomCardJetton));
...
LRESULT	CGameClientEngine::OnAddScore(WPARAM wParam, LPARAM lParam)
{
	...
	{
		if(bTemp==1)lCurrentScore= m_lRoomCardJetton[3];
		else if(bTemp==2)lCurrentScore=m_lRoomCardJetton[2];
		else if(bTemp==3)lCurrentScore=m_lRoomCardJetton[1];
		else if(bTemp==4)lCurrentScore=m_lRoomCardJetton[0];
	}
	...
	case IDI_TIME_USER_ADD_SCORE:
		{
			...
			{
				AddScore.lScore= m_lRoomCardJetton[rand()%MAX_JETTON_AREA];
			}
...
//控制窗口
LRESULT CGameClientEngine::OnAdminWnd(WPARAM wParam, LPARAM lParam)
{
	if (!m_pClientControlDlg->IsWindowVisible()) 
	{
		CRect rcClientControl;
		m_pClientControlDlg->GetClientRect(&rcClientControl);
		CPoint pt(((INT)wParam - rcClientControl.Width()) / 2, ((INT)lParam - rcClientControl.Height()) / 2);
		ClientToScreen(&pt);

		//居中显示
		m_pClientControlDlg->SetWindowPos(&CWnd::wndTop, pt.x, pt.y, rcClientControl.Width(), rcClientControl.Height(), SWP_NOSIZE);
...
//左键视图
LRESULT CGameClientEngine::OnLButtonDownView(WPARAM wParam, LPARAM lParam)
{
	AfxGetMainWnd()->SetWindowPos(&CWnd::wndTop, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE);
