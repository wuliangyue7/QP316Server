...
VOID CGameClientView::RectifyControl(int nWidth, int nHeight)
{
	...
	DeferWindowPos(hDwp,m_btStart,NULL,nWidth/2+255,nHeight-110,0,0,uFlags);
	//控制按钮 
...
VOID CGameClientView::DrawGameView(CDC * pDC, int nWidth, int nHeight)
{
	...
	for (BYTE i=0;i<GAME_PLAYER;i++)
	{
		...
		if (pIClientUserItem!=NULL)
		{
			...
			//用户金币	
			LONGLONG lLeaveScore = 0L;
			if (!m_bIsAllowAvertCheat)
			{
				lLeaveScore=pIClientUserItem->GetUserScore()-(pIClientUserItem->GetUserStatus()!=US_PLAYING?0:m_lTableScore[i]);
			}
			else
			{
				lLeaveScore=pIClientUserItem->GetUserScore();
			}
